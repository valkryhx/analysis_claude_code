...
"""
改进版专利撰写多智能体系统
修复日志记录问题，确保所有内容都能正确保存
包含详细的技术流程、Mermaid图和伪代码
"""
...
class ImprovedPatentSystem:
    ...
    def __init__(self, glm_api_key: str):
        ...
        logger.info("="*80)
        logger.info("改进版专利撰写多智能体系统初始化完成")
    ...
    async def execute_patent_workflow(self, topic: str, description: str) -> Dict[str, Any]:
        ...
        workflow_results["writing"] = flow_result
        ...
        # 保存专利文档
        await self._save_patent_document(final_patent, workflow_results)
...
请以专业的专利撰写标准进行撰写，确保内容的高质量和专业性。"""
    ...
    def _combine_workflow_results(self, workflow_results: Dict[str, Any]) -> str:
        ...
        # 添加专利标题
        combined_content.append("# 基于证据图增强的检索增强生成系统及方法")
        combined_content.append("")
        
        # 添加专利摘要
        if "writing" in workflow_results and workflow_results["writing"].get("success"):
            writing_content = workflow_results["writing"]["result"]["patent_writing"]
            # 提取摘要部分
            if "专利摘要" in writing_content:
                start_idx = writing_content.find("专利摘要")
                end_idx = writing_content.find("##", start_idx)
                if end_idx == -1:
                    end_idx = len(writing_content)
                abstract = writing_content[start_idx:end_idx].strip()
                combined_content.append(abstract)
                combined_content.append("")
        
        # 添加背景技术
        if "novelty" in workflow_results and workflow_results["novelty"].get("success"):
            novelty_content = workflow_results["novelty"]["result"]["novelty_analysis"]
            combined_content.append("## 背景技术")
            combined_content.append(novelty_content)
            combined_content.append("")
        
        # 添加发明内容
        if "research" in workflow_results and workflow_results["research"].get("success"):
            research_content = workflow_results["research"]["result"]["technical_research"]
            combined_content.append("## 发明内容")
            combined_content.append(research_content)
            combined_content.append("")
        
        # 添加具体实施方式
        if "architecture" in workflow_results and workflow_results["architecture"].get("success"):
            arch_content = workflow_results["architecture"]["result"]["system_architecture"]
            combined_content.append("## 具体实施方式")
            combined_content.append(arch_content)
            combined_content.append("")
        
        # 添加技术流程
        if "flow_analysis" in workflow_results and workflow_results["flow_analysis"].get("success"):
            flow_content = workflow_results["flow_analysis"]["result"]["technical_flow"]
            combined_content.append("## 技术流程")
            combined_content.append(flow_content)
            combined_content.append("")
        ...
        if "writing" in workflow_results and workflow_results["writing"].get("success"):
            writing_content = workflow_results["writing"]["result"]["patent_writing"]
            # 提取权利要求部分
            if "权利要求" in writing_content:
                start_idx = writing_content.find("权利要求")
                claims = writing_content[start_idx:].strip()
                combined_content.append("## 权利要求")
                combined_content.append(claims)
                combined_content.append("")
    ...
    async def _save_patent_document(self, final_patent: Dict[str, Any], workflow_results: Dict[str, Any]):
        """保存专利文档"""
        filename = f"Improved_Patent_With_Diagrams_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        
        try:
            with open(filename, 'w', encoding='utf-8') as f:
                f.write("=" * 100 + "\n")
                f.write("改进版专利撰写多智能体系统生成的专利文档\n")
    ...
    async def _save_workflow_summary(self, workflow_results: Dict[str, Any]):
        """保存工作流程摘要"""
        summary_filename = f"improved_workflow_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        
        try:
            with open(summary_filename, 'w', encoding='utf-8') as f:
                f.write("=" * 100 + "\n")
                f.write("改进版专利撰写多智能体系统工作流程摘要\n")
                f.write("=" * 100 + "\n\n")
...
async def main():
    ...
    # 创建改进版专利撰写多智能体系统
    system = ImprovedPatentSystem(config.get_glm_api_key())
